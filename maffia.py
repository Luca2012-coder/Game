...
def compute_base_level(xp):
    for lo, hi, title in LEVEL_THRESHOLDS:
        if lo <= xp < hi:
            return (lo, title)
    return (LEVEL_THRESHOLDS[-1][0], LEVEL_THRESHOLDS[-1][2])

def get_rank(xp, special_rank=None):
    if special_rank:
        return special_rank
    return compute_base_level(xp)[1]
...
if st.button("Login / Maak account"):
    ...
    st.write(f"💰 Geld: €{p['money']} | ⭐ XP: {p['xp']} | Stad: {p['city']}")
    ...
    for prof in profiles:
        rang = get_rank(prof["xp"], prof.get("special_rank"))
